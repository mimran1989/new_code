...
export default class ProvusGrid {
	...
	sectionNameRenderer(...args) {
		const [instance, td, row, , , value] = args;
		Handsontable.renderers.TextRenderer.apply(this, args);

		if (value) {
			td.innerHTML = `${'&emsp;'.repeat(instance.getSourceDataAtRow(row).$treeLevel)}${ProvusGrid.sanitizeHTML(value)}`;

			const quoteId = instance.getSourceDataAtRow(row).collaborationQuoteId;
			const quoteName = instance.getSourceDataAtRow(row).collaborationQuoteName;
			if (quoteId) {
				td.innerHTML += ` - <a href="/${quoteId}" target="_blank" style="color: #FFFFFF"><u>${quoteName}</u></a>`;
			}

			// TODO: conditionally check the row type before adding the sectionHeader class in case we lose the section
	...
	static sanitizeHTML(htmlString): string {
		const sanitationContainer = document.createElement('div');
		sanitationContainer.textContent = htmlString;
		return sanitationContainer.innerHTML;
	}

	static async forFrame(frame: any): Promise<ProvusGrid> {
	...
	async connectParentToFrame(frame: any) {
		...
		if (this.frame) {
			...
			const messageResolver = connectionResolver(deferred);
	...
	async init(options: any) {
		const deProxiedOptions = cloneDeep(options);
		if (this.frame) {
...
function connectionResolver(deferred) {
	const resolverFn = function resolveConnection({ data }: any) {
		if (data === 'host-connection-acknowledged') {
			deferred.resolve();
			window.removeEventListener('message', resolverFn);
		}
	};
	...
	return {
		get(grid: any, propertyName: any): any {
			...
			if (propertyName !== 'then') {
				result = (...args: any[]) => grid.invoke(propertyName, args);
			}
...
window.onmessage = function handleMessages({ data }: any) {
