...
export default class QuoteService extends LightningElement {
	...
	async qSaveLines() {
		...
		if (updateRequests.length) {
			...
			if (results) {
				this.mergeItemsByLines(results, contextQuoteItems);
			}
	...
	mergeItemsByLines(results, contextQuoteItems) {
		this.mergeItemsBy(
			JSON.parse(results),
	...
	async qSaveNamedRanges() {
		...
		Object.keys(changedNamedRanges).forEach((namedRangeId) => {
			...
			isTotals = isTotals || (elementDO.type === 'Quote');
			...
			contextNamedRanges[namedRange.$id] = namedRange;
	...
	async qSaveSections() {
		...
		if (updateRequests.length) {
			try {
				...
				this.commitSectionRecordForProcess(results, contextSections, newSections);
			} catch (e) {
	...
	commitSectionRecordForProcess(results, contextSections, newSections) {
		if (results) {
