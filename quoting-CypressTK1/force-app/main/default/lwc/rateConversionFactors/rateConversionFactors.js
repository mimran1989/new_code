...
export default class RateConversionFactors extends NavigationMixin(LightningElement) {
	...
	_objectOverrideVM = this.objectOverrideVM;
	...
	async renderedCallback() {
		if (this._recordId && !this.objectOverride) {
			this._componentState = this.template.querySelector('c-message-service');
			const ns = await getNamespacePrefix();
	...
	async handleSave(event) {
		...
		if (!objectOverrideChanges.hasValidChanges) {
			...
			if (commitResult.valid) {
				...
				this._componentState.publish({ key: 'ratechange', value: payload });
				this.dispatchEvent(new CloseActionScreenEvent());
			} else {
