...
import LABEL_NO_ESTIMATE_FOR_SERVICE_PRODUCT from '@salesforce/label/c.NoEstimateForServiceProduct';
...
import LABEL_UNKNONWN_ERROR from '@salesforce/label/c.UnknownError';
...
export default class ServiceRecommendations extends NavigationMixin(LightningElement) {
	...
	async refreshServiceRecommendations() {
		...
		try {
			getServiceRecommendations({ opportunityId: this.opportunityId }).then((result) => {
				this.loading = false;
				this.rerender();
				this.serviceRecommendations = Object.values(result).map((row) => ({
					...row,
					disabled: row.hasEstimate === false,
					buttonTitle: (row.hasEstimate === true) ? '' : LABEL_NO_ESTIMATE_FOR_SERVICE_PRODUCT,
				}));
	...
	handleClick(event) {
		...
		addServiceToOpportunity({ opportunityId: this.opportunityId, serviceId, productId: product.Id })
			...
			.catch((error) => {
				log(error);
				this.showNotification(LABEL_ADD_SERVICE_RECOMMENDATIONS_TITLE, LABEL_UNKNONWN_ERROR, 'error');
				this.refreshServiceRecommendations();
			});
	}
