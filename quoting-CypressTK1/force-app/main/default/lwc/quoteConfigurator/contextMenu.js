...
import LABEL_MAKE_ACTIVITIES_TASKS from '@salesforce/label/c.ActivitiesAndTasks';
	...
	constructor(context) {
		this.items = {
			...
			remove_row: {
				...
				hidden: IFrameCallback(isRemoveRowHidden),
			},
			recurring_hours: {
				name: LABEL_ADD_RECURRING_HOURS,
				hidden: IFrameCallback(isRemoveRowHidden),
				callback: (...args) => context.handleOpenRecurringHoursDialog(...args),
			},
			collaborate: {
			...
			edit_section: {
				...
				hidden: IFrameCallback(isEditSectionHidden),
				callback: (...args) => context.openNameModal(...args),
			...
			show_activities: {
				name: LABEL_MAKE_ACTIVITIES_TASKS,
				hidden: IFrameCallback(isShowActivitiesAndTasksHidden),
				callback: (...args) => context.showResourceRoleActivities(...args),
			},
			import_resources: {
				name: LABEL_IMPORT_RESOURCES,
				hidden: IFrameCallback(isImportResourcesHidden),
				callback: (...args) => context.handleShowImportResources(...args),
			},
		};
	}
}

function isShowActivitiesAndTasksHidden() {
	...
	const sourceData = this.getSourceDataAtRow(fromRow);

	return cellMeta.isSectionHeader || sourceData?.locked || sourceData?.hideActivities;
}
...
function isEditSectionHidden() {
	...
	return !cellMeta.isSectionHeader || sourceData?.locked;
}
...
function isImportResourcesHidden() {
	const selectedRange = this.getSelectedRangeLast();
	const fromRow = selectedRange.from.row;
	if (fromRow !== selectedRange.to.row) {
		return true;
